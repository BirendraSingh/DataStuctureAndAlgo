...
class DigitalWalletTransaction {
	...
	private final String INVALID_AMOUNT = "Amount should be greater than zero";
	...
	public void addMoney(DigitalWallet digitalWallet, int amount) throws TransactionException{
		if(digitalWallet.getUserAccessCode() != null && !digitalWallet.getUserAccessCode().isEmpty()){
			if(amount<=0){
				throw new TransactionException(INVALID_AMOUNT, "INVALID_AMOUNT");
			} else {
	...
	public void payMoney(DigitalWallet digitalWallet, int amount) throws TransactionException{
		if(digitalWallet.getUserAccessCode() != null && !digitalWallet.getUserAccessCode().isEmpty()){
			int avilableBalance = digitalWallet.getWalletBalance();
			if(amount > avilableBalance) {
				throw new TransactionException(INSUFFICIENT_BALANCE, "INSUFFICIENT_BALANCE");
			} else {
				digitalWallet.setWalletBalance(avilableBalance - amount); 
			}
		} else {
			throw new TransactionException(USER_NOT_AUTHORIZED, "USER_NOT_AUTHORIZED");
		}
...
class TransactionException extends Exception {
	...
	public TransactionException(String erroMessage, String errorCode) {
		super();
